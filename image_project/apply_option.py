...
def merge_image():
    ...
    try:
        ...
        if img_width >-1:
            # width 값 변경
            image_sizes=[(int(img_width),int(img_width * x.size[1]/x.size[1])) for x in images]
        ...
        # 계산식
        # 100*60 이미지를 width 80으로 변경하면 height는?
        # xy' = x'y 이므로 y'= x'y/ x -> 이식을 적용
        # 100:60 = 80:x -> 100x = 4800 = x = 4800/100
        # 우리 코드에서 x = images.size[0] , y= images.size[1], x'는 img_width, y'= img_height
        
        widths, heights = zip(*(image_sizes)) # 리스트 풀어서 담기
